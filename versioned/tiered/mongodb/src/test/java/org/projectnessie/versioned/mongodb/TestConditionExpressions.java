...
class TestConditionExpressions {
  ...
  void equalsExpression() {
    ...
    Assertions.assertEquals("{\"foo\": true}", expressionFunction.accept(BsonValueVisitor.VALUE_VISITOR));
  }
  ...
  private static void equals(String expected, ConditionExpression input) {
    Assertions.assertEquals(BsonDocument.parse(expected), MongoDBStore.transform(input));
  }

  private static void failsUnsupportedOperationException(ConditionExpression input) {
    Assertions.assertThrows(UnsupportedOperationException.class, () -> MongoDBStore.transform(input));
  }
}
